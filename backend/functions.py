...
def chat_with_ai(user_message: str):
    """
    Sends message to Google Gemini API using the new google.genai SDK.
    ...
    try:
        ...
        if not api_key:
            return "I am the GlobalPath AI assistant. (Note: Gemini API Key not found. Please set GEMINI_API_KEY environment variable to chat with real AI). How can I help you with scholarships today?"
        ...
        if faqs_df is not None and not faqs_df.empty:
            # Assuming FAQs has 'Question' and 'Answer' columns
            # We take first 50 rows to avoid token limits if large, or all if small
            context_list = []
...
AGENT DATA RULES:
6. YOU CANNOT recommend specific agents, say "Agent X is the best", or guarantee any agent is 100% safe.
7. YOU CAN explain that "Trust Scores" are calculated based on audit history and success rates.
8. YOU CAN explain that agents are listed only if they are "Active" and have a Trust Score >= 70.
9. YOU CAN explain that agents are removed if their score drops below 70 or their status changes to Inactive.
10. ALWAYS advise students to verify agent credentials independently.

KNOWLEDGE BASE:
{context}
"""
        # Format prompt with context
        full_instruction = system_instruction.format(context=context_str)
        
        # 3. Generate Content
        # We pass the instruction as system instruction or part of prompt. 
        # For simplicity with this client, we'll prepend it.
        # Ideally use config if supported by specific model version, but prepending is robust.
